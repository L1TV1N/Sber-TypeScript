...
- Каждый элемент OutputItem обязан содержать ПОЛНЫЙ набор ключей из target schema.
...
- В КАЖДОМ объекте результата должны присутствовать ВСЕ поля из OutputItem без исключений.
- Нельзя пропускать ключи даже если значение неизвестно.
- Сначала создай полный объект resultItem: OutputItem со всеми полями.
- Для каждого поля из OutputItem задай значение либо из колонок файла, либо null.
...
- Для строковых полей без надёжного значения используй null, а не пропуск поля.
- После mapping обязательно возвращай объект, где перечислены все поля OutputItem.
